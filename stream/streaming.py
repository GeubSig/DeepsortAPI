...
def stream(source, is_in):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            ###### 새로 추가 한 선
            # start_point = (w - 600, 0)
            # end_point = (w - 600, h - 300)
            # cv2.line(im0, start_point, end_point, color, thickness=2)
            # start_point = (w - 800, 0)
            # end_point = (w - 800, h - 300)
            # cv2.line(im0, start_point, end_point, color, thickness=2)
            # start_point = (w - 800, h - 360)
            # end_point = (w - 400, h - 360)
            # cv2.line(im0, start_point, end_point, color, thickness=2)
            # start_point = (w - 800, h - 480)
            # end_point = (w - 400, h - 480)
            # cv2.line(im0, start_point, end_point, color, thickness=2)
            ######
...
def count_obj_in(box, w, h, id, cls):
    ...
    # if (w - 800) < x < (w - 400):
    #     if
    if (w - 800) < x < (w - 600) and (h - 480) < y < (h - 360) and cls == 1 and id not in data1 and id not in counting_obj1:  # 첫번째에서 인식

        data1.add(id)
    # if (w - 800) < int(box[0] + (box[2] - box[0]) / 2) < (w - 400) and int(
    #         box[1] + (box[3] - box[1]) / 2) < (h - 300) and cls == 0 and id not in data1 and id not in counting_obj1:  # 첫번째에서 인식
    #     data1.add(id)
    if int(box[0] + (box[2] - box[0]) / 2) < (w - 900) and cls == 1 and id in data1 and id not in counting_obj1:  # 한번 확인이 된 아이디가 아니면 카운팅
        count1 += 1
    ...
    # if datetime.datetime(now.year, now.month, now.day, 15, 33, 0) < now < datetime.datetime(now.year, now.month, now.day, 16, 33, 0) and breakfast is False:
    #     # TODO 한시간 블로킹 개버그
    ...
    #     data1 = data2 = set()
    # elif datetime.datetime(now.year, now.month, now.day, 12, 40, 0) < now < datetime.datetime(now.year, now.month, now.day, 13, 40, 0) and lunch is False:
    ...
    #     data1 = data2 = set()
    # # 저녁 시간 저장
    # elif datetime.datetime(now.year, now.month, now.day, 18, 25, 0) < now < datetime.datetime(now.year, now.month, now.day, 19, 25, 0) and dinner is False:
...
def count_obj_out(box, w, h, id, cls):
    ...
    if (w - 800) < center_coordinates[0] < (w - 400) and center_coordinates[1] < (h - 400) and cls == 0 and id not in data2 and id not in counting_obj2:  # 첫번째에서 인식
        data2.add(id)
    if center_coordinates[0] < (w - 900) and center_coordinates[1] < (h-300) and cls == 0 and id in data2 and id not in counting_obj2:  # 한번 확인이 된 아이디이면 카운팅
        count2 += 1
...
#Pycharm 디버그모드로 실행시키세요 ㅋㅋ 아니면 i9 13세대 박든가 ㄹㅇㅋㅋ
